...
function ContactForm() {
  ...
  // MODIFICATION: This function now submits and redirects.
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    setLoading(true)
    ...
    try {
      ...
      if (res.ok) {
        // 3. <-- MODIFIED: Redirect on success
        router.push("/c2/thankyou")
      } else {
        alert("❌ Failed to send message. Please try again later.")
      }
    } catch (error) {
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6">
            ...
            <InfoItem
              icon={Clock}
              title="Business Hours"
              text="Mon–Sat: 9:00 AM – 6:00 PM\Sunday: 10:00 AM – 4:00 PM"
            />
          </div>
Still have questions about Sarjapur Area?
