    ...
    <section className="relative w-full bg-black border-y border-[#333] overflow-hidden">
      ...
      <div className="block md:hidden">
        <div className="relative w-full aspect-[4/3] overflow-hidden">
          <img
            src="https://ik.imagekit.io/j0xzq9pns/RRl%20website%20mobile%20banners%20(400%20x%20300%20px)/HOME%20HERO%202.png?updatedAt=1766849908404"
...
const FloorPlansSection = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>, planId: string) => {
    ...
    // Extract values for CRM (must be done before async ops)
    const rawData = Object.fromEntries(formData.entries());
    const nameVal = rawData['name'] as string ?? ''
    const phoneVal = rawData['phone'] as string ?? ''
    const emailVal = rawData['email'] as string ?? ''
    ...
    try {
      ...
      fetch('/enquire-now-api.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: nameVal, phone: phoneVal, email: emailVal, project: `RRL Palacio - Unit ${planId}` }),
      }).catch(console.error)
...
export default function PalacioPage() {
  ...
  // CRM-wrapped enquiry submit
  const handlePalacioEnquireSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const form = e.currentTarget
    const nameVal = (form.elements.namedItem('name') as HTMLInputElement)?.value ?? ''
    const phoneVal = (form.elements.namedItem('phone') as HTMLInputElement)?.value ?? ''
            ...
            >
              ...
              {state.succeeded ? (
                ...
                <>
                  ...
                  <form onSubmit={handlePalacioEnquireSubmit} className="space-y-4">
