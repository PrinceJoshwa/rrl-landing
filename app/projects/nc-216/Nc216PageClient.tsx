...
const floorPlansData: FloorPlan[] = [
  {
    ...
    facing: "East",
    area: "600 - 750",
    dims: { living: "10'0\" x 14'0\"", kitchen: "7'0\" x 7'0\"", masterBed: "11'0\" x 11'0\"" },
    image: "https://placehold.co/800x800/111/d9a406?text=NC+216+1BHK+Plan"
  },
  {
    id: "A1",
    type: "2BHK",
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            ...
            <motion.div variants={fadeIn} className="text-xl text-gray-300 max-w-xl mb-10 leading-relaxed space-y-2">
              <p className="flex items-center gap-2"><MapPin className="text-[#d9a406] w-5 h-5" /> Sarjapura – Chikka Tirupathi Road</p>
              <p className="text-2xl font-bold text-white">Starting from <span className="text-[#d9a406]">₹46 Lakhs*</span></p>
              <p className="text-sm text-gray-500 italic">*Limited Inventory. EOI Open Now.</p>
            </motion.div>
    ...
    <div className="container mx-auto px-4">
      ...
      >
        {[
          { icon: Award, title: "Early Bird Price", desc: "Launch prices will increase. Lock in the best rate now." },
          { icon: Zap, title: "High Growth", desc: "Located in a rapidly developing corridor of South Bangalore." },
          { icon: ShieldCheck, title: "Affordable", desc: "Top-tier 1, 2 & 3 BHK units at affordable rates." }
        ].map((item, idx) => (
          <motion.div key={idx} variants={fadeIn} className="group">
            <Card className="bg-black/50 border border-white/10 p-8 h-full hover:border-[#d9a406]/50 transition-colors duration-300 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-24 h-24 bg-[#d9a406]/5 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-150"></div>
  ...
  const handleUnlockSubmit = async (e: React.FormEvent<HTMLFormElement>, configType: string) => {
    ...
    try {
      ...
      fetch('/enquire-now-api.php', {
        ...
        body: JSON.stringify({ name: nameVal, phone: phoneVal, email: emailVal, project: `RRL NC216 - ${configType}` }),
      }).catch(console.error)
  ...
  const configurations = [
    {
      type: "1 BHK",
      tag: "Coveted",
      description: "Smart living with efficient layouts",
      features: ["Optimized Space", "Modern Kitchen", "1 Bathroom"],
      gradient: "from-[#d9a406]/20 via-transparent to-transparent",
    },
    {
      type: "2 BHK",
                ...
                <div className="mt-auto pt-8">
                  ...
                  {submitted[config.type] ? (
                    <div className="text-center py-4">
                      <CheckCircle className="w-10 h-10 text-green-500 mx-auto mb-3" />
                      <p className="text-white font-bold">Thank You!</p>
                      <p className="text-gray-400 text-sm">We'll share the floor plans shortly.</p>
                    </div>
                  ) : activeForm === config.type ? (
                    <form
                      onSubmit={(e) => handleUnlockSubmit(e, config.type)}
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>, planId: string) => {
    ...
    try {
      ...
      fetch('/enquire-now-api.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: nameVal, phone: phoneVal, email: emailVal, project: `RRL NC216 - Unit ${planId}` }),
      }).catch(console.error)
    ...
    <section className="py-24 bg-[#050505]" id="floorplans">
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div>
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Unit <span className="text-[#d9a406]">Configurations</span></h2>
            <p className="text-gray-400 max-w-xl">Premium 1, 2 & 3 BHK Apartments designed for maximum space utilization. <br /> Submit EOI to view detailed layouts.</p>
          </div>
          <div className="flex gap-2 bg-[#111] p-1 rounded-lg border border-white/10">
...
export default function Nc216Page() {
  ...
  const handleNc216Submit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    ...
    fetch('/enquire-now-api.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: nameVal, phone: phoneVal, email: emailVal, project: 'RRL NC216' }),
    }).catch(console.error)
          ...
          >
            ...
            >
              ...
              {state.succeeded ? (
                ...
                <>
                  <div className="text-center mb-8"><h3 className="text-3xl font-bold text-white mb-2">Submit EOI</h3><p className="text-[#d9a406] text-sm">Early Bird Pricing Ends Soon!</p></div>
                  <form onSubmit={handleNc216Submit} className="space-y-4">
                    <input type="hidden" name="project" value="NC 216 General Enquiry" />
                    <div className="space-y-2">
                      <input
                        name="name"
