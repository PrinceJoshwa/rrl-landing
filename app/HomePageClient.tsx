...
// // //     try {
// // //       // 1. Formspree (primary – controls UX flow)
...
// // //       // 2. Paramantra CRM – fire and forget
// // //       fetch('/download-brochure-api.php', {
// // //         method: 'POST',
// // //         headers: { 'Content-Type': 'application/json' },
// // //         body: JSON.stringify({ name: formData.name, phone: formData.phone, email: formData.email, project: 'RRL Palm Altezze' }),
// // //       }).catch(console.error)

// // //       if (res.ok) {
// // //         // Auto-download the brochure
// // //         const link = document.createElement("a")
// // //         link.href = "/PalmAltezze.pdf"
// // //         link.download = "RRL Palm Altezze.pdf"
...
// // //   const [formData, setFormData] = useState({ name: "", email: "", contact: "", property: "", message: "" })
// // //   const router = useRouter()
...
// // //                     className="min-h-[100px] bg-gray-100 focus:border-gold-500"
// // //                   />
// // //                 </div>
// // //                 <Button
// // //                   type="submit"
...
// //           transition={{ duration: 1.2, ease: "easeOut" }}
// //           viewport={{ once: true }}
// //           // ADDED: max-w-[1536px] and mx-auto
// //           // This stops the image from getting bigger than its native resolution
...
// //             alt="RRL Hero Banner Desktop"
// //             loading="eager"
// //             // REVERTED: Back to w-full h-auto so absolutely nothing gets cropped
// //             className="w-full h-auto object-contain"
// //           />
// //         </motion.div>
// //       </div>
...
// //       mobileImage: "https://ik.imagekit.io/j0xzq9pns/Project/RRL%20project%20explore%20(366%20x%20256%20px)/RRL%20Palm%20Altezze.png?updatedAt=1766849725863",
// //       bedrooms: "2, 3 BHK",
// //       devSize: "1 acre 38 Guntas",
// //       totalUnits: "115 Units",
// //       timeline: "Possession: 2027",
// //       timelineSub: "Next Handover Phase"
// //     },
// //     {
...
// //       bedrooms: "2, 3 BHK",
// //       devSize: "1.5 Acres",
// //       totalUnits: "148 Units",
// //       timeline: "Early possession granted",
// //       timelineSub: null
// //     },
// //   ]
...
// // ========== 2. HERO FIRST SECTION (Replaced old Hero) ==========
// function HeroFirstSection() {
//   const [state, handleSubmit] = useForm("xkgkyavn"); // Preserved your Formspree ID
//   const [isVRModalOpen, setIsVRModalOpen] = useState(false)
...
//                   <p className="text-white font-medium">Request Sent Successfully!</p>
//                 </div>
//               ) : (
...
//                     className="w-full bg-[#d9a406] hover:bg-white text-black font-bold h-14 rounded-xl transition-all shadow-lg mt-2"
//                   >
//                     {state.submitting ? "Sending..." : "Submit Details"} <Send className="ml-2 w-4 h-4" />
//                   </Button>
...
//   viewport={{ once: true }}
//   // REDUCED SIZE: Changed max-w to 220px on mobile, 240px on desktop. 
//   // Added a thicker border (#222) so it looks like an elegant smartphone frame.
...
//     // Added scale-[1.05] to slightly zoom in and hide default YouTube black bars/borders
...
//                   <p className="text-gray-400 text-sm mt-2">We will contact you shortly.</p>
//                 </div>
//               ) : (
...
//                     type="submit" 
//                     disabled={state.submitting}
//                     className="w-full bg-[#d9a406] hover:bg-white text-black font-bold h-14 rounded-xl transition-all duration-300 mt-2 shadow-[0_0_20px_rgba(217,164,6,0.15)]"
//                   >
//                     {state.submitting ? "Processing..." : "Request Virtual Tour"}
//                   </Button>
//                 </form>
//               )}
//             </div>
...
// ========== NEW: HERO FIRST SECTION (Below Banner) ==========
function HeroFirstSection() {
...
// ========== ORIGINAL UNTOUCHED HERO SECTION ==========
function Hero() {
  const [currentSlide, setCurrentSlide] = useState(0)
  const [isPlaying, setIsPlaying] = useState(false);
  const [isVRModalOpen, setIsVRModalOpen] = useState(false)
...
{/* Grand CTA Button - Combines both button widths into one */}
              ...
              <div className="pt-8 border-t border-gray-800 flex flex-col gap-8">
                ...
                <div className="flex flex-wrap items-center gap-6">
                  <div className="flex items-center gap-3">
                    <div className="flex -space-x-1">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className="h-5 w-5 fill-[#d9a406] text-[#d9a406]" />
                      ))}
                    </div>
                    <span className="text-sm text-gray-400 font-medium">
                      4.9/5 Customer Rating
            ...
            >
              <div className="relative aspect-video overflow-hidden rounded-[28px] border border-[#d9a406]/20 shadow-[0_0_40px_rgba(217,164,6,0.08)] group">
                ...
                <div className="absolute bottom-5 left-5 right-5 z-20 flex items-center justify-between">
                  <div>
                    <p className="text-white text-xl font-bold">RRL Builders & Developers</p>
                    <p className="text-gray-300 text-sm">Beyond Homes. A Lifestyle.</p>
                  </div>
                  <div className="rounded-full bg-[#d9a406] p-3 shadow-[0_0_20px_rgba(217,164,6,0.4)]">
                    <Play className="w-5 h-5 text-black fill-black" />
                  </div>
              ...
              <div className="relative overflow-hidden rounded-[30px] border border-[#d9a406]/20 group shadow-[0_0_40px_rgba(217,164,6,0.08)]">
                <div className="relative h-[500px] w-full overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-8 z-20">
                    <div className="inline-flex items-center gap-2 rounded-full border border-[#d9a406]/30 bg-black/60 backdrop-blur-md px-5 py-2 mb-5">
                      <Award className="w-4 h-4 text-[#d9a406]" />
                      <span className="text-xs uppercase tracking-[0.3em] text-[#d9a406] font-semibold">Global Recognition</span>
                    </div>
                    <h3 className="text-3xl md:text-4xl font-bold leading-tight text-white font-playfair">
                      Times Real Estate <span className="block text-[#d9a406]">Icon Awards 2023</span>
                    </h3>
                  </div>
                </div>
              </div>
...
// ========== 4. VR TOUR PALM SECTION (With Fixed Shorts Dimensions) ==========
function VRTourPalm() {
  const [state, handleSubmit] = useForm("xkgkyavn"); // Make sure to replace with your ID
            ...
            <div className="bg-[#111]/80 backdrop-blur-xl p-8 md:p-10 rounded-[2rem] border border-[#d9a406]/20 shadow-2xl relative overflow-hidden">
              ...
              {state.succeeded ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <Input name="name" placeholder="Full Name" required className="bg-black/50 border-neutral-800 text-white h-14 rounded-xl" />
                  <Input name="email" type="email" placeholder="Email Address" required className="bg-black/50 border-neutral-800 text-white h-14 rounded-xl" />
                  <Input name="phone" type="tel" placeholder="Mobile Number" required className="bg-black/50 border-neutral-800 text-white h-14 rounded-xl" />
                  <Button type="submit" disabled={state.submitting} className="w-full bg-[#d9a406] hover:bg-white text-black font-bold h-14 rounded-xl transition-all">
...
// ========== REST OF THE SECTIONS PRESERVED EXACTLY ==========
function PalmAltezzeBanner() {
  return (
    <section className="relative w-full bg-black py-20">
      <div className="absolute inset-0 z-0">
...
function HorizontalBrochureForm() {
  const router = useRouter()
  const [formData, setFormData] = useState({ name: "", email: "", phone: "", whatsappUpdates: false })
  const [loading, setLoading] = useState(false)
      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
          <div className="lg:w-1/3 text-center lg:text-left">
            ...
            <p className="text-gray-400 mt-2">Get the complete floor plans, pricing, and amenities list instantly.</p>
          </div>

          <form onSubmit={handleDownloadSubmit} className="lg:w-2/3 w-full">
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <Input type="text" name="name" placeholder="Your Name" value={formData.name} onChange={handleInputChange} required className="h-12 bg-black/50 text-white border-gold-700/50 focus:border-gold-500 w-full" />
              <Input type="email" name="email" placeholder="Your Email" value={formData.email} onChange={handleInputChange} required className="h-12 bg-black/50 text-white border-gold-700/50 focus:border-gold-500 w-full" />
              <Input type="tel" name="phone" placeholder="Mobile Number" value={formData.phone} onChange={handleInputChange} required maxLength={10} className="h-12 bg-black/50 text-white border-gold-700/50 focus:border-gold-500 w-full" />
              <Button type="submit" disabled={loading} className="h-12 w-full md:w-auto px-8 bg-gold-500 hover:bg-gold-600 text-black font-bold whitespace-nowrap">
                {loading ? "Processing..." : "Download Now"}
              </Button>
            </div>
            <div className="flex items-center space-x-2 mt-3 justify-center lg:justify-start">
              <Checkbox id="whatsapp-updates-hor" name="whatsappUpdates" checked={formData.whatsappUpdates} onCheckedChange={(checked) => setFormData((prev) => ({ ...prev, whatsappUpdates: !!checked }))} className="border-gold-700 data-[state=checked]:bg-gold-500" />
              <label htmlFor="whatsapp-updates-hor" className="text-sm text-gray-400 cursor-pointer">I agree to receive updates via WhatsApp</label>
            </div>
          </form>
        </div>
      </div>
