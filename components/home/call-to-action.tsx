...
export function CallToAction() {
  ...
  // UPDATED: handleSubmit function to integrate with Formspree
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()

    // Simple validation for property
    ...
    try {
      const response = await fetch("https://formspree.io/f/manawezn", { // <-- ⚠️ REPLACE WITH YOUR FORMSPREE ID
        method: "POST",
  ...
  return (
    <section className="py-24 bg-slate-900 text-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-20 left-20 w-80 h-80 bg-amber-400 rounded-full blur-3xl animate-float"></div>
        <div
          className="absolute bottom-20 right-20 w-96 h-96 bg-blue-40Animate-float"
          style={{ animationDelay: "3s" }}
        ></div>
      </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <Select
                    value={formData.property}
                    onValueChange={(value) => setFormData({ ...formData, property: value })}
                    required // Added required to the Select component
                  >
                    <SelectTrigger className="bg-white/10 border-white/20 text-white">
                      <SelectValue placeholder="Which Property are you looking for?" />
                    </SelectTrigger>
